...
namespace Engine.cgimin.material.alphacylce
{
    public class AlphaCycleMaterial : BaseMaterial
    {

        private int modelviewProjectionMatrixLocation;
        private int alphaValueLocation;
        private int cycleLocation;

        private Stopwatch stopWatch;

        public AlphaCycleMaterial()
        {
            // Shader-Programm wird aus den externen Files generiert...
            CreateShaderProgram(MATERIAL_DIRECTORY + "alphacycle/AlphaCycle_VS.glsl",
                                MATERIAL_DIRECTORY + "alphacycle/AlphaCycle_FS.glsl");
            ...
            cycleLocation = GL.GetUniformLocation(Program, "cycle");

            stopWatch = new Stopwatch();
            stopWatch.Start();
        }

        public void Draw(BaseObject3D object3d, int textureID, float alpha, float cyclespeed, BlendingFactorSrc sourceBlendFunc = BlendingFactorSrc.SrcAlpha, BlendingFactorDest destBlendFunc = BlendingFactorDest.OneMinusSrcAlpha)
        {
            ...
            GL.Uniform1(cycleLocation, stopWatch.ElapsedMilliseconds * cyclespeed);
        ...
        {
            Draw(object3d, settings.colorTexture, settings.alpha, settings.textureAnimationSpeed, settings.blendFactorSource, settings.blendFactorDest);
        }
