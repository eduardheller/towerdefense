...
{
    class ParticleTeslaHit : ParticleSystem
    {
        private float _scaleFactor = 0.0f;

        public ParticleTeslaHit(ParticleAtlas textureAtlas, float pps, float speed, float gravity, float lifetime) : 
        ...
        public override void EmitParticle(FrameEventArgs e, Vector3 position, Vector3 velocity)
        {
            ...
            ParticleRenderer.AddParticle(new Particle(new Vector3(position), vel, TextureAtlas, Gravity, Lifetime, 0, 2f));
        }
    }
}
