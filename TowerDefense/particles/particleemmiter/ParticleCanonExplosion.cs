...
{
    class ParticleCanonExplosion : ParticleSystem
    {
        public ParticleCanonExplosion(ParticleAtlas textureAtlas, float pps, float speed, float gravity, float lifetime) : 
        ...
        public override void EmitParticle(FrameEventArgs e, Vector3 position, Vector3 velocity)
        {
            ...
            Vector3 vel = new Vector3(dirx, Math.Abs(diry), dirx);
            vel = Vector3.Multiply(vel, Speed);
            ParticleRenderer.AddParticle(new Particle(new Vector3(position), vel, TextureAtlas, Gravity, Lifetime, rot, 0.4f));
        }
    }
}
